...
fn build_vendored(link_mode: LinkMode) {
    ...
    // Always provide an architecture target, including for native builds.
    // Otherwise Zig detects the build runner's CPU and may emit host-specific
    // AVX-family instructions that are unavailable on another x86-64 machine.
    // Release artifacts must use Zig's portable baseline for the Rust target.
    ...
    match link_mode {
        ...
        LinkMode::Static => {
            ...
            if target.contains("windows") && target.contains("msvc") {
                ...
                // windows-gnu: zig installs COFF archives under MSVC-style
                // names; GNU ld wants `libghostty-vt.a`. Same format, so an
                // aliased copy satisfies the lookup.
                if target.contains("windows") {
                    for dir in &search_dirs {
                        let source = dir.join("ghostty-vt-static.lib");
                        let alias = dir.join("libghostty-vt.a");
                        if source.exists() && !alias.exists() {
                            std::fs::copy(&source, &alias)
                                .expect("aliasing the static archive for GNU ld");
                        }
                    }
                }
