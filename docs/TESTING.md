...
| Launch planning | native profiles and explicit Windows-to-WSL command construction; real shell directories after remote OSC 7 reports, restored local reports with encoded spaces, unavailable hostless directories, and fish prompt recovery for opening/restarting worktrees after remote directory output without a local `cd` (when fish is installed) |
...
and `github-auth-collapsed` for selected-PR readiness, confirmation, empty,
file-scrolling, and auth states. `github-loading`, `github-refreshing`, and
`github-pull-requests-loading` cover the tab-specific animated loading shells.
