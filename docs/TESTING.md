...
| Session picker | first-running selection, stopped-session resume rejection, keyboard/footer traversal, startup dismissal, visible prompt ownership, cancel-default end/remove confirmation, and partial cleanup failures preserving surviving records |
...
`session-picker-empty`, `session-picker-error`, and `session-picker-many`.
